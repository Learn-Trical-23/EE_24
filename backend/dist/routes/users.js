...
usersRouter.get("/", authenticate, requireRole(["super_admin"]), async (_req, res) => {
    const users = await query("SELECT id, full_name, email, role FROM profiles ORDER BY created_at DESC");
    res.json(users);
});
// Change user role (for super admin)
usersRouter.post("/:userId/role", authenticate, requireRole(["super_admin"]), async (req, res) => {
