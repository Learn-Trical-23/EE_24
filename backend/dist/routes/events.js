...
eventsRouter.post("/", authenticate, requireRole(["admin", "super_admin"]), async (req, res) => {
    ...
    try {
        const result = await pool.query(`INSERT INTO events (title, datetime, mention_date, module, kind, created_by) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id, title, datetime, mention_date, module, kind, created_by, created_at`, [title, datetime, mention_date || null, module || "", kind || "other", res.locals.userId]);
...
eventsRouter.put("/:id", authenticate, requireRole(["admin", "super_admin"]), async (req, res) => {
    ...
    const { title, datetime, mention_date, module, kind } = req.body;
    try {
        const result = await pool.query(`UPDATE events SET title = $1, datetime = $2, mention_date = $3, module = $4, kind = $5 WHERE id = $6 RETURNING id, title, datetime, mention_date, module, kind, created_by, created_at`, [title, datetime, mention_date || null, module || "", kind || "other", id]);
        res.json({ data: result.rows[0] });
    }
    catch (err) {
        console.error(err);
