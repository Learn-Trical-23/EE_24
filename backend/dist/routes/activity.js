...
activityRouter.get("/latest", authenticate, requireRole(["admin", "super_admin"]), async (_req, res) => {
    ...
    const activities = await query(`SELECT a.id, a.type, a.material_id, a.title, a.description, a.section_key, a.week_label, a.file_url, a.timestamp, p.full_name, p.email
     FROM (
